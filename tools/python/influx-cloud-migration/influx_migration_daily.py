...
def report_bucket_series_count(bucket_name, host, token, org_name=None, skip_verify=False):
    try:
        with InfluxDBClient(url=host, token=token, verify_ssl=not skip_verify) as client:
            ...
            logging.warning(
                f"Series count metric not found for bucket '{bucket_name}' in {host}/metrics. "
                "Skipping series count check."
                )
        ...
        logging.warning(f"Failed to get series count for bucket with name {bucket_name} in {host}")

def backup_csv(backup_path, root_token, src_host, bucket_name=None, full=False, skip_verify=False, src_org=None, start_date=None, end_date=None):
    """
    ...
    try:
        ...
        # Backup a single bucket
            ...
            bucket_org = client.organizations_api().find_organization(bucket['org_id']).name
            write_bucket_to_csv_by_day(backup_path=backup_path, token=root_token, bucket=bucket, org_name=bucket_org, host=src_host, skip_verify=skip_verify, start_date=start_date, end_date=end_date)
    #         write_bucket_to_csv(backup_path=backup_path, token=root_token, bucket=bucket, org_name=bucket_org, host=src_host, skip_verify=skip_verify)
...
def parse_args(args):
    ...
    parser.add_argument("--allow-unowned-s3-bucket", help="Optional. Whether to automatically allow the "
        ...
        default=False, required=False, action='store_true')    
    parser.add_argument("--start-date", help="Optional. The date to start the migration at, in the format "
        "YYYY-MM-DD. If not provided, the migration will start at the beginning of the bucket.",
        required=False)
    parser.add_argument("--end-date", help="Optional. The date to end the migration at, in the format "
        "YYYY-MM-DD. If not provided, the migration will end at the end of the bucket.",
        required=False)
...
def write_bucket_from_csv(path, host, token, bucket_name, new_bucket_name=None, org=None, skip_verify=False):
    ...
    try:
        ...
        dest_bucket_name = new_bucket_name or bucket_name
        logging.info(f"Restoring to bucket {dest_bucket_name}")

        retention_rules_raw = metadata.get("retention_rules", [])

        if len(retention_rules_raw) > 0:
            retention_seconds = retention_rules_raw[0].get("every_seconds", 0)
            shard_group_retention_seconds = retention_rules_raw[0].get("shard_group_duration_seconds", None)
        else:
            retention_seconds = 0
            shard_group_retention_seconds = None
        ...
        if retention_seconds > 0:
            retention_rules = BucketRetentionRules(type='expire', every_seconds=retention_seconds)
            if shard_group_retention_seconds:
        ...
        if retention_rules:
            ...
            client.buckets_api().create_bucket(bucket_name=dest_bucket_name, org=org_name)
...
def write_bucket_from_csv_by_day(path, host, token, bucket_name, new_bucket_name=None, org=None, skip_verify=False):
    """
    날짜별로 분할된 CSV 파일을 순차적으로 restore합니다.
    """
    import glob
    try:
        # 메타데이터 로드
        ...
        # 버킷 생성 (최초 1회만)
        client = InfluxDBClient(url=host, token=token, verify_ssl=not skip_verify, timeout=MILLISECOND_TIMEOUT, org=org_name)
        if not bucket_exists(host, token, dest_bucket_name, org_name, skip_verify):
            client.buckets_api().create_bucket(bucket_name=dest_bucket_name, org=org_name)
        client.close()

        # 날짜별 CSV 파일 반복
        for csv_file in sorted(glob.glob(f'{path}/bucket_{bucket_name}_*.csv')):
            write_command = [
                'influx', 'write', '--host', host, '-t', token, '--bucket', dest_bucket_name,
                '--org', org_name, '--format', 'csv', '--file', csv_file
            ]
            if skip_verify:
                write_command.append("--skip-verify")
            proc = subprocess.run(write_command, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def write_bucket_to_csv_by_day(backup_path, bucket, org_name, token, host, skip_verify=False, start_date=None, end_date=None):
    """
    날짜별로 데이터를 쿼리해서 CSV로 저장합니다.
    """
    try:
        # Write metadata to json
        bucket['org_name'] = org_name
        ...
        if start_date is None or end_date is None:
            # 전체 기간: 1678-01-01 ~ 2800-01-01 (기존과 동일)
            start_date = datetime(1678, 1, 1)
            end_date = datetime(2800, 1, 1)
        else:
        ...
        for single_date in daterange(start_date, end_date):
            day_start = single_date.strftime("%Y-%m-%dT00:00:00Z")
            day_end = (single_date + timedelta(days=1)).strftime("%Y-%m-%dT00:00:00Z")
            # csv_filename = f'{backup_path}/bucket_{bucket["name"]}_{single_date.strftime("%Y%m%d")}.csv'
            csv_filename = f'{backup_path}/{bucket["name"]}_{single_date.strftime("%Y_%m_%d")}.csv'
            with open(csv_filename, 'w') as csv_file:
                query_command = [
                    'influx', 'query',
                    f'from(bucket: "{bucket["name"]}") |> range(start: {day_start}, stop: {day_end})',
                    '--host', host, "-t", token, "--org", org_name, "--raw"
                ]
                if skip_verify:
                    query_command.append("--skip-verify")
...
def daterange(start_date, end_date):
    """
    start_date부터 end_date 전날까지 하루씩 증가시키며 date 객체를 yield합니다.
    """
